# SolidFrame Example: Digital Wallet (Symfony + Event Sourcing + CQRS)
...
| `solidframe/symfony` | SolidFrameBundle, DbalEventStore, DbalSnapshotStore, handler discovery |
...
│   └── Query/
│       ├── ListAccounts.php + Handler           → reads from projection table (DBAL)
│       ├── GetAccount.php + Handler             → reads from projection table (DBAL)
│       ├── GetTransactions.php + Handler        → reads from projection table (DBAL)
│       └── GetBalanceAt.php + Handler           ← Temporal Query (replays events to date)
│
├── Controller/
│   └── AccountController.php                    (#[Route] attributes, RequestValidator)
│
├── EventSubscriber/
│   └── ExceptionSubscriber.php                  (KernelEvents::EXCEPTION → JSON responses)
│
├── Http/
│   └── RequestValidator.php                     (Symfony Validator constraints)
│
├── Infrastructure/
│   ├── Persistence/
│   │   ├── EventSourcedAccountRepository.php    (EventStore + Projection dispatch)
│   │   └── Dbal/
│   │       ├── ConnectionFactory.php            (SQLite/PostgreSQL/MySQL)
│   │       └── SchemaManager.php                (Doctrine DBAL Schema)
│   ├── Projection/
...
│       └── RebuildProjectionsCommand.php        ← app:projection:rebuild
│
...
- **Projection Rebuild** — `app:projection:rebuild` replays all events from the store to rebuild read models from scratch
...
GET /api/accounts/{id}
  → GetAccountHandler
    → Connection.fetchAssociative('account_balances')   ← reads from projection (fast, no replay)
```
...
### How It Differs from laravel-event-sourced

| Aspect | laravel-event-sourced | symfony-event-sourced |
|---|---|---|
| Framework | Laravel 13 | Symfony 8.0 |
| Query handlers | `DB` facade | Doctrine DBAL `Connection` |
| Projections | `DB` facade | Doctrine DBAL `Connection` |
| Validation | FormRequest classes | `RequestValidator` + Symfony Validator |
| Exception handling | Laravel middleware | `ExceptionSubscriber` (KernelEvents) |
| Routing | `routes/api.php` | `#[Route]` attributes |
| Schema | Laravel migrations | `SchemaManager` (Doctrine DBAL Schema) |
| Console commands | Artisan `$signature` | `#[AsCommand]` attribute |
| EventStore wiring | `AppServiceProvider` | `services.yaml` + `SolidFrameBundle` |
| Domain/Application | Identical (framework-agnostic) | Identical (framework-agnostic) |
...
curl -X POST http://localhost:8005/api/accounts \
...
# Deposit money
curl -X POST http://localhost:8005/api/accounts/{id}/deposit \
  -H "Content-Type: application/json" \
  -d '{"amount": 10000, "description": "Salary"}'

# Withdraw money
curl -X POST http://localhost:8005/api/accounts/{id}/withdraw \
  -H "Content-Type: application/json" \
  -d '{"amount": 5000, "description": "ATM"}'

# Transfer between accounts
curl -X POST http://localhost:8005/api/accounts/{source-id}/transfer \
  -H "Content-Type: application/json" \
  -d '{"target_account_id": "{target-id}", "amount": 15000, "description": "Rent"}'

# View transaction history
curl http://localhost:8005/api/accounts/{id}/transactions

# Check balance at a past date (temporal query)
curl http://localhost:8005/api/accounts/{id}/balance-at?date=2026-04-10

# Rebuild projections from event store
php bin/console app:projection:rebuild
```
...
**56 tests, 103 assertions** covering:
...
- Architecture: Domain/Application layer isolation from Infrastructure/Controller/Symfony, DDD + CQRS presets, value object/event immutability, exception contracts
...
5. **Projection Rebuild** — Console command to replay all events and rebuild read models from scratch
...
8. **SolidFrameBundle** — DbalEventStore, DbalSnapshotStore, handler auto-discovery
9. **PHPStan level 8** — Full static analysis with solidframe/phpstan-rules, zero errors
