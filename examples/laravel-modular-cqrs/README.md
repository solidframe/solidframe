# SolidFrame Example: E-Commerce (Laravel + Modular CQRS + Saga)
...
| `solidframe/laravel` | ModuleServiceProvider, SagaStore, handler discovery |
...
│   │
│   ├── OrderModule.php                      ← ModuleInterface
│   ├── OrderServiceProvider.php             ← ModuleServiceProvider (routes, migrations, DI)
│   ├── routes.php
│   └── Database/Migrations/
│
...
│   │
│   ├── Infrastructure/Persistence/Eloquent/
│   ├── Http/ (ProductController, AddProductRequest)
│   ├── InventoryModule.php
...
- **Modular Monolith** — Each bounded context is a self-contained module with its own Domain, Application, Infrastructure, and HTTP layers
...
- **Each module is a Laravel ServiceProvider** — routes, migrations, and DI bindings are self-registered via `ModuleServiceProvider`
...
| Aspect | laravel-hexagonal-cqrs | laravel-modular-cqrs |
|---|---|---|
| Structure | Single `app/` with flat Domain | Separate `modules/` per bounded context |
...
| ServiceProvider | Single AppServiceProvider | ModuleServiceProvider per module |
...
curl http://localhost:8003/api/orders/{id}

# Check payment was created
curl http://localhost:8003/api/payments/{order-id}

# Check stock was reduced
curl http://localhost:8003/api/products/{laptop-id}
```
...
# Feature tests (API + saga integration — uses SQLite)
...
**58 tests, 104 assertions** covering:
...
The `solidframe/laravel` bridge provides generator commands for modular projects:

```bash
# Scaffold a new module
php artisan make:module Shipping
# → modules/Shipping/ShippingModule.php
# → modules/Shipping/ShippingServiceProvider.php
# → modules/Shipping/Database/Migrations/

# Generate a saga
php artisan make:saga OrderFulfillment
# → app/Application/Saga/OrderFulfillmentSaga.php
...
2. **ModuleServiceProvider** — Auto-loads routes, migrations, and config per module
